...
def show_PC_time(FC):
    """显示24小时制 HH MM 纯数字时间（纯黑背景，非MIX版不透明）"""
    global State_change, sleep_event
    num_add = 3651
    if State_change == 1:
        state_change_clear()
        LCD_ADD(0, 0, SHOW_WIDTH, SHOW_HEIGHT)
        LCD_Set_Color(FC, BLACK)  # 设置前景色和纯黑背景
        LCD_Color_set(0, 0, SHOW_WIDTH, SHOW_HEIGHT, (0, 0, 0))  # 再次确保纯黑底
    ...
    # 非MIX版逐个发送（带不透明背景，不透出底层）
    LCD_ASCII_32X64(0 + 8, 8, chr((time_h // 10) + 48), num_add)
    LCD_ASCII_32X64(32 + 8, 8, chr((time_h % 10) + 48), num_add)
    LCD_ASCII_32X64(80 + 8, 8, chr((time_m // 10) + 48), num_add)
    LCD_ASCII_32X64(112 + 8, 8, chr((time_m % 10) + 48), num_add)
