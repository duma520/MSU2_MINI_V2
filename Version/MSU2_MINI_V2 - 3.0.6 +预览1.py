...
def get_window_image(hWnd=None):
    ...
    # 当窗口跨DPI缩放显示器移动或快速调整大小时，get_actual_size()可能返回过时值
    dpi_w, dpi_h = default_capture.get_actual_size()
    if width != dpi_w or height != dpi_h:
        # 尺寸不一致时以实际位图为准，并更新缓存的DPI尺寸
        default_capture.dpi_width = width
        default_capture.dpi_height = height
...
def SER_Write(Data_U0):
    ...
    if written != len(Data_U0):
        print("SER_Write: 写入不完整 expected=%d actual=%d" % (len(Data_U0), written))
...
def set_select_hwnd(hwnd):
    global config_obj, windows_combobox, screen_shot_queue, screen_process_queue, screen_frame_generation
    ...
    while dev.mg_screen_thread_running:
        ...
        try:
            ...
            if config_obj and config_obj.preview_enabled:
                with _preview_lock:
                    _preview_rgb = im1.astype(np.uint8).copy()
...
def _safe_send_rgb888(rgb888_array):
    ...
    if config_obj and config_obj.preview_enabled:
        with _preview_lock:
            _preview_rgb = rgb888_array.astype(np.uint8).copy()
    try:
...
def UI_Page():  # 进行图像界面显示
    ...
    preview_var = tk.IntVar(settings_frame, 0)
    preview_var.set(config_obj.preview_enabled)
    preview_cb = ttk.Checkbutton(
        settings_frame, text="开启实时预览（显示小屏当前内容）", variable=preview_var,
        command=change_preview
    )
    preview_cb.pack(anchor=tk.W, pady=pad_scale_xy5)
    ...
    preview_scale = 3  # 160x80 放大3倍 → 480x240
    preview_canvas = tk.Canvas(root, width=SHOW_WIDTH * preview_scale, 
                               height=SHOW_HEIGHT * preview_scale,
    ...
    def update_preview():
        ...
        if config_obj and config_obj.preview_enabled:
            ...
            if img is not None and img.size > 0:
                try:
                    # numpy (H,W,3) → PIL Image → resize → PhotoImage
                    pil_img = Image.fromarray(img, 'RGB')
                    pil_img = pil_img.resize((SHOW_WIDTH * preview_scale, 
                                              SHOW_HEIGHT * preview_scale),
                                             Image.Resampling.NEAREST)
                    tk_img = ImageTk.PhotoImage(pil_img)
                    preview_canvas.create_image(0, 0, anchor=tk.NW, image=tk_img)
                    preview_canvas.image = tk_img  # 保持引用防止GC
                except Exception:
                    pass
        window.after(200, update_preview)  # 每200ms刷新
