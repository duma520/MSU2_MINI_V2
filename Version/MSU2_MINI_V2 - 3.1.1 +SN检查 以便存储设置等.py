...
def _dump_usb_descriptor(port):
    """诊断：打印USB设备的完整描述符信息，用于确认是否有唯一序列号"""
    print("=" * 60)
    ...
    # 判断唯一标识可用性
    if port.serial_number and port.serial_number.strip():
        print("  >>> 唯一序列号:   %s (可用作设备指纹!)" % port.serial_number)
    elif port.location and port.location.strip():
        print("  >>> USB物理路径:  %s (不换口可稳定识别)" % port.location)
    else:
        print("  >>> 警告: 无可用的唯一标识, 只能依赖COM端口号")
    print("=" * 60)
