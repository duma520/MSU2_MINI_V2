...
def UI_Page():  # 进行图像界面显示
    ...
    def show_custom():
        ...
        cmd_type_list = ["p 文本", "a 锚点", "m 移动到", "t 相对移动", "f 字体", "c 颜色", "i 图片", "v 数值"]
        cmd_hints = {
        ...
        for col, (label, letter) in enumerate(add_buttons):
            ttk.Button(btn_frame1, text=label, padding=pad_scale_xy,
                       command=lambda l=letter: open_cmd_dialog(l)).grid(
                row=0, column=col, sticky=tk.NSEW, padx=pad_scale_xy5, pady=pad_scale_xy5)
