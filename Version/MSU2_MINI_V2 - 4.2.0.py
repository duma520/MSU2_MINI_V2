...
def UI_Page():  # 进行图像界面显示
    ...
    # ---- 子页4：页面内容 ----
    content_frame = ttk.Frame(settings_notebook, padding=(pad_scale_xy5 * 2, pad_scale_xy5 * 2))
    ...
    # 跑马灯实时预览（所见即所得）
    marquee_preview_canvas = tk.Canvas(content_frame, width=(SHOW_WIDTH * scale_factor // 100),
    ...
    def update_marquee_preview(*args):
        try:
            ...
            try:
                ...
                font = MiniMark.load_font("./simhei.ttf", font_size)
            draw.text((0, (SHOW_HEIGHT - font_size) // 2), text, fill=(255, 255, 255), font=font)
    ...
    weather_row = ttk.Frame(content_frame)
    weather_row.pack(anchor=tk.W, pady=pad_scale_xy5)
    ttk.Label(weather_row, text="天气城市:").pack(side=tk.LEFT)
    weather_city_var = tk.StringVar(content_frame, config_obj.weather_city)
    ...
    ttk.Label(content_frame, text="支持中文城市名，如 北京 或 Beijing", foreground="gray").pack(anchor=tk.W, pady=(0, pad_scale_xy5))
    ...
    crypto_row = ttk.Frame(content_frame)
    crypto_row.pack(anchor=tk.W, pady=pad_scale_xy5)
    ttk.Label(crypto_row, text="行情交易对:").pack(side=tk.LEFT)
    crypto_symbols_var = tk.StringVar(content_frame, config_obj.crypto_symbols)
    ...
    ping_row = ttk.Frame(content_frame)
    ping_row.pack(anchor=tk.W, pady=pad_scale_xy5)
    ttk.Label(ping_row, text="延迟测试目标:").pack(side=tk.LEFT)
    ping_host_var = tk.StringVar(content_frame, config_obj.ping_host)
    ...
    # 纪念日（每行一项：名称|MM-DD）
    ttk.Label(content_frame, text="纪念日（每行一项：名称|月-日，如 生日|01-01）:").pack(anchor=tk.W)
    memo_text = tk.Text(content_frame, height=4, width=40)
    memo_text.insert(tk.END, "\n".join(config_obj.memo_items))
    memo_text.pack(anchor=tk.W, fill=tk.X, pady=pad_scale_xy5)

    # 待办事项（每行一项）
    ttk.Label(content_frame, text="待办事项（每行一项）:").pack(anchor=tk.W)
    todo_text = tk.Text(content_frame, height=4, width=40)
...
def show_marquee():
    ...
    if x + text_w < SHOW_WIDTH:
        draw.text((x + text_w + 20, y), text, fill=(255, 255, 255), font=font)
    marquee_offset += 2
    rgb888 = np.asarray(im1, dtype=np.uint32)
    _safe_send_rgb888(rgb888)
...
def show_proc():
    ...
    for i, (name, rss) in enumerate(procs[:5]):
        draw.text((4, i * 15), "%d.%s %.0fM" % (i + 1, name[:10], rss / 1048576.0), fill=(255, 255, 255), font=font)
    rgb888 = np.asarray(im1, dtype=np.uint32)
    _safe_send_rgb888(rgb888)
...
def show_worldclock():
    ...
    zones = [("北京", 8), ("伦敦", 0), ("纽约", -5), ("东京", 9)]
    im1 = Image.new("RGB", (SHOW_WIDTH, SHOW_HEIGHT), (0, 0, 0))
    draw = ImageDraw.Draw(im1)
    font = MiniMark.load_font("./simhei.ttf", 16)
    for i, (name, offset) in enumerate(zones):
        t = now_utc + timedelta(hours=offset)
        draw.text((4, 4 + i * 19), "%-4s %02d:%02d" % (name, t.hour, t.minute), fill=(255, 255, 255), font=font)
    rgb888 = np.asarray(im1, dtype=np.uint32)
    _safe_send_rgb888(rgb888)
    dev.sleep_event.wait(1.0)
...
def show_gauge():
    ...
    draw_one(40, 52, 34, cpu, (255, 80, 80), "CPU")
    draw_one(120, 52, 34, mem, (80, 160, 255), "内存")
    rgb888 = np.asarray(im1, dtype=np.uint32)
    _safe_send_rgb888(rgb888)
    dev.sleep_event.wait(0.5)
...
def show_hwdetail():
    ...
    for name, hw, sensor, stype in items:
        hardwares.add(hw)
    hardware_monitor_manager.update_hardwares(hardwares)
    temps = [(n, hw, s, t) for n, hw, s, t in items if t == "Temperature"][:4]
    fans = [(n, hw, s, t) for n, hw, s, t in items if t == "Fan"][:1]
    im1 = Image.new("RGB", (SHOW_WIDTH, SHOW_HEIGHT), (0, 0, 0))
    draw = ImageDraw.Draw(im1)
    font = MiniMark.load_font("./simhei.ttf", 14)
    lines = []
    for n, hw, s, t in temps + fans:
...
def fetch_crypto():
    ...
    try:
        ...
        for sym in symbols[:4]:
            url = "https://api.binance.com/api/v3/ticker/price?symbol=%s" % sym
            d = _json.loads(_http_get(url))
            lines.append("%s %.2f" % (sym, float(d["price"])))
