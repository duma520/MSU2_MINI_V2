...
def show_hwdetail():
    """硬件详情：列出温度/风扇传感器（数量可配置，超过一页自动翻页）"""
    global config_obj
    dev = get_current_device()
    if dev is None:
        return
    ...
    for name, (hw, sensor) in hardware_monitor_manager.sensors.items():
        ...
        if stype in ("Temperature", "Fan"):
            items.append((name, hw, sensor, stype))
    hardwares = set()
    for name, hw, sensor, stype in items:
        hardwares.add(hw)
    hardware_monitor_manager.update_hardwares(hardwares)
    temps = [(n, hw, s, t) for n, hw, s, t in items if t == "Temperature"]
    fans = [(n, hw, s, t) for n, hw, s, t in items if t == "Fan"]
    all_items = temps + fans
    ...
    for n, hw, s, t in all_items:
        ...
        val = s.Value
        if val is None:
            lines.append("%s --" % short[:12])
        elif t == "Temperature":
            lines.append("%s %.0f°C" % (short[:12], val))
        else:
            lines.append("%s %.0fRPM" % (short[:12], val))
    if not lines:
        lines = ["未找到传感器"]
    per_page = 5
...
def show_hotsearch():
    ...
    _refresh_cache_if_needed(_hot_cache, fetch_hotsearch)
