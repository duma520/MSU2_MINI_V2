...
def UI_Page():  # 进行图像界面显示
    ...
    for v in (gauge_cpu_color_var, gauge_mem_color_var, gauge_disk_color_var,
              gauge_cpu_temp_color_var, gauge_gpu_color_var):
...
def show_gauge():
    """仪表盘：CPU/内存/磁盘使用率弧形仪表（项目与颜色可配置，超过一页自动翻页）"""
    global config_obj
    dev = get_current_device()
    if dev is None:
        return
    ...
    if config_obj.gauge_show_cpu:
        try:
            gauges.append(("CPU", psutil.cpu_percent(interval=None), hex2rgb(config_obj.gauge_cpu_color)))
        except Exception:
            gauges.append(("CPU", 0, (255, 80, 80)))
    if config_obj.gauge_show_mem:
        try:
            gauges.append(("内存", psutil.virtual_memory().percent, hex2rgb(config_obj.gauge_mem_color)))
        except Exception:
            gauges.append(("内存", 0, (80, 160, 255)))
    if config_obj.gauge_show_disk:
        try:
            gauges.append(("磁盘", psutil.disk_usage('/').percent, hex2rgb(config_obj.gauge_disk_color)))
        except Exception:
            gauges.append(("磁盘", 0, (80, 255, 80)))
    if config_obj.gauge_show_cpu_temp:
        v = _find_sensor_value("Temperature", "CPU")
        gauges.append(("CPU温度", v if v is not None else 0, hex2rgb(config_obj.gauge_cpu_temp_color)))
    if config_obj.gauge_show_gpu:
        v = _find_sensor_value("Load", "GPU")
        gauges.append(("GPU", v if v is not None else 0, hex2rgb(config_obj.gauge_gpu_color)))
    if not gauges:
        gauges = [("CPU", 0, (255, 80, 80))]
    ...
    def draw_one(cx, cy, r, percent, color, label):
        bbox = (cx - r, cy - r, cx + r, cy + r)
        draw.arc(bbox, start=180, end=360, fill=(60, 60, 60), width=6)
        if percent > 0:
            angle = int(180 * min(percent, 100) / 100)
        ...
        draw.text((cx - 18, cy + 2), "%d%%" % percent, fill=color, font=font_v)

    for i, (label, percent, color) in enumerate(gauges_page):
        cx, cy = positions[i]
        draw_one(cx, cy, 34, percent, color, label)
