...
class sys_config(object):
    def __init__(self):
        ...
        # --- 热搜 ---
        self.hotsearch_interval = 60  # 热搜刷新时间(秒)
        # --- 硬件详情监控类型 ---
...
def UI_Page():  # 进行图像界面显示
    ...
    page_setting_guide = {
        MARQUEE_PAGE_ID: ("文字跑马灯：文本、字体、字号、颜色", 4, 0),
        WEATHER_PAGE_ID: ("天气：设置城市（支持中文）", 4, 1),
        CRYPTO_PAGE_ID: ("行情：设置交易对", 4, 1),
        HOTSEARCH_PAGE_ID: ("热搜：设置刷新时间", 4, 1),
        PING_PAGE_ID: ("网络延迟：设置测试目标", 4, 1),
        TIMER_PAGE_ID: ("番茄钟：设置时长", 4, 2),
        WORLDCLOCK_PAGE_ID: ("世界时钟：设置时区", 4, 2),
        MEMO_PAGE_ID: ("纪念日：设置列表", 4, 3),
        TODO_PAGE_ID: ("待办事项：设置列表", 4, 3),
        PROC_PAGE_ID: ("进程TOP：设置显示数量", 5, None),
        HWDETAIL_PAGE_ID: ("硬件详情：设置监控类型与数量", 5, None),
        GAUGE_PAGE_ID: ("仪表盘：设置项目与颜色", 5, None),
            ...
            if pname == name:
                ...
                if guide:
                    try:
                        ...
                        if guide[2] is not None:
                            content_notebook.select(guide[2])
                    except Exception:
                        pass
                break
    ...
    def change_hotsearch_interval(*args):
        global config_obj
        try:
    ...
    hot_row = ttk.Frame(net_frame)
    hot_row.pack(anchor=tk.W, pady=pad_scale_xy5)
    ttk.Label(hot_row, text="热搜刷新时间(秒):").pack(side=tk.LEFT)
    hotsearch_interval_var = tk.IntVar(net_frame, 0)
    hotsearch_interval_var.set(config_obj.hotsearch_interval)
    ttk.Spinbox(hot_row, from_=10, to=3600, textvariable=hotsearch_interval_var, width=6).pack(side=tk.LEFT, padx=pad_scale_xy5)
    hotsearch_interval_var.trace_add("write", change_hotsearch_interval)
    ...
    # ---- 子页4.5：监控显示 ----
    monitor_frame = ttk.Frame(settings_notebook, padding=(pad_scale_xy5 * 2, pad_scale_xy5 * 2))
    ...
    ttk.Label(monitor_frame, text="硬件详情监控类型:").pack(anchor=tk.W, pady=(pad_scale_xy5, 0))
    hwdetail_type_vars = {}
    hw_type_row = ttk.Frame(monitor_frame)
    hw_type_row.pack(anchor=tk.W, pady=pad_scale_xy5)
    for t in ("Temperature", "Fan", "Voltage", "Load", "Power"):
        var = tk.IntVar(monitor_frame, t in (config_obj.hwdetail_types or "").split(","))
    ...
    # 仪表盘（项目与颜色，多于一页自动翻页）
    ttk.Label(monitor_frame, text="仪表盘显示项目与颜色:").pack(anchor=tk.W, pady=(pad_scale_xy5, 0))
    ...
    gauge_cpu_var = tk.IntVar(monitor_frame, config_obj.gauge_show_cpu)
    gauge_mem_var = tk.IntVar(monitor_frame, config_obj.gauge_show_mem)
    gauge_disk_var = tk.IntVar(monitor_frame, config_obj.gauge_show_disk)
    gauge_cpu_temp_var = tk.IntVar(monitor_frame, config_obj.gauge_show_cpu_temp)
    gauge_gpu_var = tk.IntVar(monitor_frame, config_obj.gauge_show_gpu)
    gauge_gpu_temp_var = tk.IntVar(monitor_frame, config_obj.gauge_show_gpu_temp)
    gauge_fan_var = tk.IntVar(monitor_frame, config_obj.gauge_show_fan)
    gauge_upload_var = tk.IntVar(monitor_frame, config_obj.gauge_show_upload)
    gauge_download_var = tk.IntVar(monitor_frame, config_obj.gauge_show_download)
    ...
    cpu_row = ttk.Frame(monitor_frame)
    cpu_row.pack(anchor=tk.W, pady=pad_scale_xy5)
    ttk.Checkbutton(cpu_row, text="CPU", variable=gauge_cpu_var, command=change_gauge).pack(side=tk.LEFT)
    gauge_cpu_color_var = tk.StringVar(monitor_frame, config_obj.gauge_cpu_color)
    ttk.Entry(cpu_row, textvariable=gauge_cpu_color_var, width=9).pack(side=tk.LEFT, padx=pad_scale_xy5)
    ttk.Button(cpu_row, text="颜色", padding=pad_scale_xy, command=lambda: _pick_gauge_color(gauge_cpu_color_var)).pack(side=tk.LEFT)

    mem_row = ttk.Frame(monitor_frame)
    mem_row.pack(anchor=tk.W, pady=pad_scale_xy5)
    ttk.Checkbutton(mem_row, text="内存", variable=gauge_mem_var, command=change_gauge).pack(side=tk.LEFT)
    gauge_mem_color_var = tk.StringVar(monitor_frame, config_obj.gauge_mem_color)
    ttk.Entry(mem_row, textvariable=gauge_mem_color_var, width=9).pack(side=tk.LEFT, padx=pad_scale_xy5)
    ttk.Button(mem_row, text="颜色", padding=pad_scale_xy, command=lambda: _pick_gauge_color(gauge_mem_color_var)).pack(side=tk.LEFT)

    disk_row = ttk.Frame(monitor_frame)
    disk_row.pack(anchor=tk.W, pady=pad_scale_xy5)
    ttk.Checkbutton(disk_row, text="磁盘", variable=gauge_disk_var, command=change_gauge).pack(side=tk.LEFT)
    gauge_disk_color_var = tk.StringVar(monitor_frame, config_obj.gauge_disk_color)
    ttk.Entry(disk_row, textvariable=gauge_disk_color_var, width=9).pack(side=tk.LEFT, padx=pad_scale_xy5)
    ttk.Button(disk_row, text="颜色", padding=pad_scale_xy, command=lambda: _pick_gauge_color(gauge_disk_color_var)).pack(side=tk.LEFT)

    cpu_temp_row = ttk.Frame(monitor_frame)
    cpu_temp_row.pack(anchor=tk.W, pady=pad_scale_xy5)
    ttk.Checkbutton(cpu_temp_row, text="CPU温度", variable=gauge_cpu_temp_var, command=change_gauge).pack(side=tk.LEFT)
    gauge_cpu_temp_color_var = tk.StringVar(monitor_frame, config_obj.gauge_cpu_temp_color)
    ttk.Entry(cpu_temp_row, textvariable=gauge_cpu_temp_color_var, width=9).pack(side=tk.LEFT, padx=pad_scale_xy5)
    ttk.Button(cpu_temp_row, text="颜色", padding=pad_scale_xy, command=lambda: _pick_gauge_color(gauge_cpu_temp_color_var)).pack(side=tk.LEFT)

    gpu_row = ttk.Frame(monitor_frame)
    gpu_row.pack(anchor=tk.W, pady=pad_scale_xy5)
    ttk.Checkbutton(gpu_row, text="GPU", variable=gauge_gpu_var, command=change_gauge).pack(side=tk.LEFT)
    gauge_gpu_color_var = tk.StringVar(monitor_frame, config_obj.gauge_gpu_color)
    ttk.Entry(gpu_row, textvariable=gauge_gpu_color_var, width=9).pack(side=tk.LEFT, padx=pad_scale_xy5)
    ttk.Button(gpu_row, text="颜色", padding=pad_scale_xy, command=lambda: _pick_gauge_color(gauge_gpu_color_var)).pack(side=tk.LEFT)

    gpu_temp_row = ttk.Frame(monitor_frame)
    gpu_temp_row.pack(anchor=tk.W, pady=pad_scale_xy5)
    ttk.Checkbutton(gpu_temp_row, text="GPU温度", variable=gauge_gpu_temp_var, command=change_gauge).pack(side=tk.LEFT)
    gauge_gpu_temp_color_var = tk.StringVar(monitor_frame, config_obj.gauge_gpu_temp_color)
    ttk.Entry(gpu_temp_row, textvariable=gauge_gpu_temp_color_var, width=9).pack(side=tk.LEFT, padx=pad_scale_xy5)
    ttk.Button(gpu_temp_row, text="颜色", padding=pad_scale_xy, command=lambda: _pick_gauge_color(gauge_gpu_temp_color_var)).pack(side=tk.LEFT)

    fan_row = ttk.Frame(monitor_frame)
    fan_row.pack(anchor=tk.W, pady=pad_scale_xy5)
    ttk.Checkbutton(fan_row, text="风扇", variable=gauge_fan_var, command=change_gauge).pack(side=tk.LEFT)
    gauge_fan_color_var = tk.StringVar(monitor_frame, config_obj.gauge_fan_color)
    ttk.Entry(fan_row, textvariable=gauge_fan_color_var, width=9).pack(side=tk.LEFT, padx=pad_scale_xy5)
    ttk.Button(fan_row, text="颜色", padding=pad_scale_xy, command=lambda: _pick_gauge_color(gauge_fan_color_var)).pack(side=tk.LEFT)

    upload_row = ttk.Frame(monitor_frame)
    upload_row.pack(anchor=tk.W, pady=pad_scale_xy5)
    ttk.Checkbutton(upload_row, text="上传", variable=gauge_upload_var, command=change_gauge).pack(side=tk.LEFT)
    gauge_upload_color_var = tk.StringVar(monitor_frame, config_obj.gauge_upload_color)
    ttk.Entry(upload_row, textvariable=gauge_upload_color_var, width=9).pack(side=tk.LEFT, padx=pad_scale_xy5)
    ttk.Button(upload_row, text="颜色", padding=pad_scale_xy, command=lambda: _pick_gauge_color(gauge_upload_color_var)).pack(side=tk.LEFT)

    download_row = ttk.Frame(monitor_frame)
    download_row.pack(anchor=tk.W, pady=pad_scale_xy5)
    ttk.Checkbutton(download_row, text="下载", variable=gauge_download_var, command=change_gauge).pack(side=tk.LEFT)
    gauge_download_color_var = tk.StringVar(monitor_frame, config_obj.gauge_download_color)
    ttk.Entry(download_row, textvariable=gauge_download_color_var, width=9).pack(side=tk.LEFT, padx=pad_scale_xy5)
    ttk.Button(download_row, text="颜色", padding=pad_scale_xy, command=lambda: _pick_gauge_color(gauge_download_color_var)).pack(side=tk.LEFT)
...
def _find_sensor_value(stype, keyword=""):
    """从硬件监控管理器查找传感器值（stype: 传感器类型, keyword: 硬件名关键字，大写）"""
    if hardware_monitor_manager is None or hardware_monitor_manager == 1:
        return None
    try:
        for name, (hw, sensor) in hardware_monitor_manager.sensors.items():
            s = str(sensor.SensorType)
            if s == stype and (not keyword or keyword in hw.Name.upper()):
                return sensor.Value
    except Exception:
        pass
    return None
...
def show_gauge():
    ...
    if config_obj.gauge_show_cpu_temp:
        v = _find_sensor_value("Temperature", "CPU")
        gauges.append(("CPU温度", v if v is not None else 0, hex2rgb(config_obj.gauge_cpu_temp_color), "temp"))
    if config_obj.gauge_show_gpu_temp:
        v = _find_sensor_value("Temperature", "GPU")
        gauges.append(("GPU温度", v if v is not None else 0, hex2rgb(config_obj.gauge_gpu_temp_color), "temp"))
    if config_obj.gauge_show_gpu:
        v = _find_sensor_value("Load", "GPU")
        gauges.append(("GPU", v if v is not None else 0, hex2rgb(config_obj.gauge_gpu_color), "pct"))
    if config_obj.gauge_show_fan:
        v = _find_sensor_value("Fan")
        gauges.append(("风扇", v if v is not None else 0, hex2rgb(config_obj.gauge_fan_color), "fan"))
...
def show_hwdetail():
    ...
    hardwares = set()
    for name, hw, sensor, stype in items:
        hardwares.add(hw)
    hardware_monitor_manager.update_hardwares(hardwares)
    all_items = items
    max_n = max(1, int(config_obj.hwdetail_max))
    all_items = all_items[:max_n]
...
def show_hotsearch():
    ...
    lines = _hot_cache.get("data") or ["获取中…"]
    im1 = Image.new("RGB", (SHOW_WIDTH, SHOW_HEIGHT), (0, 0, 0))
    draw = ImageDraw.Draw(im1)
    font = MiniMark.load_font("./simhei.ttf", 14)
    for i, line in enumerate(lines[:5]):
        draw.text((4, i * 15), ("%d.%s" % (i + 1, line)), fill=(255, 255, 255), font=font)
    rgb888 = np.asarray(im1, dtype=np.uint32)
    _safe_send_rgb888(rgb888)
    dev.sleep_event.wait(0.5)
