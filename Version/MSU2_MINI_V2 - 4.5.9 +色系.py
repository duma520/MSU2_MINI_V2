...
def UI_Page():  # 进行图像界面显示
    ...
    def _make_scheme_row(frame, vars_):
        """生成一个"配色方案"下拉行：选择方案后把颜色按顺序套用到 vars_ 的颜色输入框"""
        row = ttk.Frame(frame)
        ...
        ttk.Label(row, text="选择后自动套用到下方颜色（按顺序）", foreground="gray").pack(side=tk.LEFT)

        def _on_apply(event=None):
            ...
            for i, v in enumerate(vars_):
                v.set(colors[i % len(colors)])
            insert_text_message("已应用配色方案：%s" % var.get())

        combo.bind("<<ComboboxSelected>>", _on_apply)
    ...
    # ==== 子子页：配色方案（管理） ====
    scheme_frame = ttk.Frame(monitor_notebook, padding=(pad_scale_xy5 * 2, pad_scale_xy5 * 2))
    monitor_notebook.add(scheme_frame, text="  配色方案  ")
