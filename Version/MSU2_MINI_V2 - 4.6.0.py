...
class ApiHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _apply_screen(self, data, device=None):
        ...
        if "image" in data:  # base64 编码的 PNG/JPG/BMP 图像
            raw = _b64.b64decode(data["image"])
            img = Image.open(_io.BytesIO(raw)).convert("RGB")
            img = _api_resize_image(img, data.get("fit", "stretch"))
            api_set_frame(np.asarray(img, dtype=np.uint8), device)
        elif "rgb888" in data:  # base64 编码的原始 RGB888 字节（W*H*3）
        ...
        elif "pixels" in data and width and height:  # 扁平 RGB 列表
    ...
    def _post_page(self, data, device=None):
        try:
            ...
            if dev is None:
                self._send_json({"ok": False, "error": "no device"}, 400)
                return
            set_active_device_config(dev)
            ...
            if pid in PAGE_ID:
                ...
                self._send_json({"ok": True, "page": PAGE_ID.get(pid, str(pid))})
            else:
                self._send_json({"ok": False, "error": "无效页面，可用 /api/pages 查看"}, 400)
        except Exception as e:
    ...
    def _post_mirror(self, data, device=None):
        ...
        try:
            ...
            if hwnd == 0:
                self._send_json({"ok": False, "error": "需要 hwnd"}, 400)
                return
            set_active_device_config(dev)
            config_obj.select_window_hwnd = hwnd
            ...
            if dev:
                ...
                clear_queue(dev.screen_process_queue)
            state_change_set(save=True)
            self._send_json({"ok": True, "hwnd": hwnd, "page": PAGE_ID.get(SCREEN_PAGE_ID, "")})
        except Exception as e:
            self._send_json({"ok": False, "error": str(e)}, 400)

    def _post_slideshow(self, data, device=None):
        """多图轮播投屏：images 为 base64 图像数组，interval 为间隔秒"""
        try:
            raws = data.get("images") or []
            if not isinstance(raws, list) or not raws:
                self._send_json({"ok": False, "error": "需要 images 数组（base64 图像）"}, 400)
                return
            try:
            ...
            self._send_json({"ok": True, "count": len(frames), "interval": interval})
        except Exception as e:
            self._send_json({"ok": False, "error": str(e)}, 400)

    def _post_stop(self, device=None):
        """停止指定屏投屏：停止轮播 + 清屏 + 回到 API 投屏页（等待接入提示）"""
        api_stop_slideshow(device)
        api_set_frame(None, device)
        try:
            dev = device if device is not None else get_current_device()
            ...
            pass
        self._send_json({"ok": True, "page": PAGE_ID.get(API_PAGE_ID, "API投屏")})
...
def _build_openapi_doc():
    ...
    return {
        ...
        "info": {
            ...
            "description": "USB 副屏工具的外部接入接口。其他程序可通过 REST / WebSocket 自定义投屏内容（图像、文本、清屏、切页、实时帧）。屏幕分辨率 %dx%d。%s"
...
def start_api_server():
    """启动本地 API 服务器（HTTP + WebSocket），仅监听 127.0.0.1"""
    global _api_server
