...
def UI_Page():  # 进行图像界面显示
    ...
    def on_device_select(event=None):
        ...
        for dev in all_devices.values():
            if dev.device_name == name and dev.device_state == 1:
                # 保存旧设备页面到其自身配置（不读全局 config_obj，避免被 daemon 线程串扰）
                if old is not None and old != dev:
                    if old.config is not None:
                        old.config.state_machine = old.state_machine
                    else:
                ...
                set_active_device_config(dev)
                # 把新设备的运行时页面同步到其自身配置（不经过全局 config_obj）
                if dev.config is not None:
                    dev.config.state_machine = getattr(dev, "state_machine", SCREEN_PAGE_ID)
                state_change_set(save=False)
                sync_page_combobox()
                ...
                try:
                    ...
                    pass
                # 更新镜像窗口下拉框（无条件同步，避免残留上屏值）
                desc = get_hwnd_desc(config_obj.select_window_hwnd)
                windows_combobox.set(desc if desc else "")
                break
    ...
    def _apply_settings_ui_to_config():
        ...
        global config_obj, _ui_batch_sync
        _ui_set_active()  # 锁定到UI当前选中设备，避免daemon线程切换config_obj导致刷新错乱
        _ui_batch_sync = True
        try:
            # ---- 按页面导航 ----
    ...
    def change_api(*args):
        ...
        if _ui_batch_sync:
            # 切换设备批量刷新控件期间：只同步当前屏的API配置，不重启服务器
            # （否则每次切换都会 stop/start 全部API服务器，造成日志重复与卡顿）
            return
        save_config()
        try:
            stop_api_server()
    ...
    def refresh_hw_info():
        """刷新设备信息（后台线程读取数据，主线程更新界面，避免串口读取卡住UI）"""
        for f in (hw_conn_frame, hw_sfr_frame, hw_flash_frame, hw_parts_frame, hw_sys_frame):
            for w in f.winfo_children():
                w.destroy()
        _add_info_row(hw_conn_frame, "读取中", "请稍候…")

        def _collect():
            """后台线程：收集设备/系统信息（SFR读取为串口I/O，避免阻塞UI）"""
            dev = get_current_device()
            ...
            return dev, usb, fw, connected, sfr, sfr_rows, sys_rows, flash

        def _apply(result):
            """主线程：用收集到的数据重建界面"""
            try:
                dev, usb, fw, connected, sfr, sfr_rows, sys_rows, flash = result
                for f in (hw_conn_frame, hw_sfr_frame, hw_flash_frame, hw_parts_frame, hw_sys_frame):
                    for w in f.winfo_children():
                        w.destroy()
                # ---- 子页1：连接信息（USB描述符） ----
                ...
                if sfr:
                    ...
                    for name, row in sfr_rows:
                        _add_info_row(hw_sfr_frame, name, row)
                else:
                ...
                for label, val in sys_rows:
                    _add_info_row(hw_sys_frame, label, val)
            except Exception:
                pass

        threading.Thread(target=lambda: window.after(0, lambda: _apply(_collect())), daemon=True).start()
    ...
    def combo_configure(event):
        # 兼容传入事件对象或控件本身（后台线程枚举后刷新时传控件）
        combo = getattr(event, 'widget', event)
        ...
        def apply_cameras():
            ...
            try:
                if len(all_cameras) > 1:
                    PAGE_ID[CAMERA_VIDEO_ID] = (PAGE_ID_EN[CAMERA_VIDEO_ID] if config_obj.language == "English"
                                                 else PAGE_ID_CN[CAMERA_VIDEO_ID])
                ...
                camera_combobox['values'] = list(all_cameras.keys())
                sync_page_combobox()
            except Exception:
                pass
        window.after(0, apply_cameras)

    threading.Thread(target=load_cameras_async, daemon=True).start()
        ...
        def _worker():
            ...
            def _done():
                ...
                try:
                    ...
                    if desc:
                        widget.set(desc)
                    widget["values"] = sorted(wins.keys(), key=str.lower)
                    combo_configure(widget)
                except Exception:
                    pass
            window.after(0, _done)
    ...
    def _apply_main_ui_to_config():
        ...
        global config_obj, lcd_size_var, _ui_batch_sync
        _ui_set_active()  # 锁定到UI当前选中设备，避免daemon线程切换config_obj导致刷新错乱
        _ui_batch_sync = True
        try:
            # 文字颜色滑块 + 颜色预览
            ...
            interval_var.set(config_obj.photo_interval_var + config_obj.second_times)
            fps_var.set(config_obj.fps_var)
            # 相机（无条件同步为当前设备配置；找不到时清空，避免残留上屏值）
            ...
            # 镜像窗口（无条件同步；窗口无效时清空，避免残留上屏值）
            desc = get_hwnd_desc(config_obj.select_window_hwnd)
            win32_windows_var.set(desc if desc else "")
            # 页面/方向下拉
            sync_page_combobox()
            sync_lcd_combobox()
        except Exception as e:
        ...
        try:
            ...
            if dev is not None and dev.config is not None:
                ...
                if key != last_synced_device_state:
                    ...
                    sync_page_combobox()
                    sync_lcd_combobox()
                # 设备连接/配置切换后，同步刷新主控页与设置页控件，
                # 避免「强制投屏」等设置在设备连接晚于UI创建时仍显示默认值（实际配置已保存）
                sig = (dev.device_name, id(dev.config), dev.config_file)
