...
# 兼容遗留：部分外围函数（拖放路径/快捷键等）仍引用 tk 常量
...
def sync_page_combobox():
    ...
    if cb is not None:
        try:
            ...
            cb.clear()
            cb.addItems(list(PAGE_ID.values()))
            dev = get_current_device()
...
# 三层结构：第一层「主控|关于」→ 第二层「屏幕1|屏幕2|屏幕3…」→ 第三层「主控|设置|设备信息」
...
#              自定义内容/上翻下翻/方向/页面/相机/屏幕镜像窗口/分辨率/实时预览），每屏一套
    ...
    # ---------- 主控页（第二层：每屏标签 + 底部共享信息框） ----------
    root = QWidget()
    top_nb.addTab(root, "  主控  ")
...
# 布局精确复刻老版 4.7.1 主控页：左列烧写区/控制区，右列颜色/填充/间隔/FPS/相机，底部实时预览
        ...
        def _update_preview():
            ...
            if cc.preview_enabled:
                try:
                    ...
                    pass
            QTimer.singleShot(200, _update_preview)
        ...
        def refresh_hw_info():
            ...
            def _worker():
                ...
                if _result is not None:
                    QTimer.singleShot(0, lambda: _apply(_result))
    ...
    def _rebuild_main_tabs():
        ...
        for d in connected:
            if d.index not in _main_ctxs:
                ...
                _build_settings_tab(settings_page, d)
                hw_page = QWidget()
                inner_nb.addTab(hw_page, "  设备信息  ")
                _build_hw_tab(hw_page, d)
    ...
    def _activate_by_name(name):
        ...
        global _primary_device, _active_main_dev_id, _syncing_screen_tabs
