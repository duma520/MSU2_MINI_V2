...
class sys_config(object):
    def __init__(self):
        ...
        # --- 电视墙（显示墙布局） ---
        self.wall_rows = 0  # 显示墙纵向行数（0=自动按屏数推荐）
        self.wall_cols = 0  # 显示墙横向列数（0=自动按屏数推荐）
...
# 布局精确复刻老版 4.7.1 主控页：左列烧写区/控制区，右列颜色/填充/间隔/FPS/相机，底部实时预览
    ...
    def _wall_save():
        """保存显示墙布局到配置"""
        try:
            config_obj.wall_rows = wall_rows_sb.value()
            config_obj.wall_cols = wall_cols_sb.value()
            save_config()
        except Exception:
            pass
    ...
    # 初始：从配置恢复布局（若保存过）
    try:
        _wr = int(getattr(config_obj, "wall_rows", 0) or 0)
        _wc = int(getattr(config_obj, "wall_cols", 0) or 0)
    except Exception:
        _wr = _wc = 0
    if _wr >= 1 and _wc >= 1:
