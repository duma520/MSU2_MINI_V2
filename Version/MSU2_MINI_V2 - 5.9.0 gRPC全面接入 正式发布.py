...
<div id="panel-docs" class="panel">
  ...
  <p><b>接入协议：</b>HTTP/WebSocket/SSE（端口N）、TCP（JSON行，N+1）、UDP（JSON报，N+2）、ZeroMQ（REP，N+3，需pyzmq）、gRPC（N+4，msu2.Msu2Api，需grpcio）、Windows命名管道（<code>\\.\pipe\MSU2_MINI_V2_api</code>）、Unix Domain Socket（api_unix.sock）、热文件夹（程序目录 hotfolder/，放入图片/文本即投屏）。<br>
...
def stop_api_extra():
    """停止 TCP / UDP / 热文件夹 / Unix Socket / ZeroMQ / gRPC（命名管道为 daemon 线程随程序退出结束）"""
...
class sys_config(object):
    def __init__(self):
        ...
        self.api_protocols = "tcp,hotfolder"  # 附加接入协议（逗号分隔：tcp/udp/hotfolder/pipe/unix/zmq/grpc）；http/ws 随 api_enable 总开关
